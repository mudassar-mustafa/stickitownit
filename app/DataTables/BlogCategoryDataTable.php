...
class BlogCategoryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()->eloquent($query)
            ...
            ->addColumn('name', function ($category) {
                return empty($category->name) ? "None" : $category->name;
            })
            ->addColumn('status', function ($category) {
                return ($category->status == "active") ? '<span class="badge rounded-pill btn btn-success">Active</span>' : '<span class="badge rounded-pill btn btn-danger">De-active</span>';
            })
            ->escapeColumns(
                []
            )
            ->addColumn('action', function ($category) {
                ...
                $deleteAction .= Form::open(['action' => ['App\Http\Controllers\Backend\BlogCategoryController@destroy', $category->id],
                    'method' => 'DELETE', 'id' => 'chargeDelete' . $category->id . '']);
    ...
    {
        return 'Blog-Categories' . date('YmdHis');
    }
}
