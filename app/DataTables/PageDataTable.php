    ...
    public function dataTable($query)
    {
        return datatables()->eloquent($query)
            ...
            ->addColumn('name', function ($page) {
                return empty($page->name) ? "None" : $page->name;
            })
            ->addColumn('status', function ($page) {
                return ($page->status == "active") ? '<span class="badge rounded-pill btn btn-success">Active</span>' : '<span class="badge rounded-pill btn btn-danger">De-active</span>';
            })
            ->escapeColumns(
                []
            )
            ->addColumn('action', function ($page) {
                ...
                $editAction = '<a href="' . route("backend.pages.page.edit", $page->id) . '" class="item-edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit font-small-4"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>';

                $deleteAction = '<a form-alert-message="Kindly Confirm the removal of this Page" ' .
                    'form-id="chargeDelete' . $page->id . '" class="deleteModel" href="'
                    . route('backend.pages.page.destroy', $page->id) . '" data-toggle="tooltip" ' .
                    'data-original-title="Remove Page"> ' .
                ...
                $deleteAction .= Form::open(['action' => ['App\Http\Controllers\Backend\PageController@destroy', $page->id],
                    'method' => 'DELETE', 'id' => 'chargeDelete' . $page->id . '']);
